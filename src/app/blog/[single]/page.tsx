...
const PostSingle = async ({ params }: { params: { single: string } }) => {
  ...
  const req = await fetch(`${apiUrl}/posts?_fields=title,content,date,featured_media&slug=${params.single}`);
  const single = await req.json();

  const reqMedia = await fetch(`${apiUrl}/media/${single[0].featured_media}?_fields=source_url`);
  const feature_image = await reqMedia.json();
  single[0].featured_media = feature_image.source_url;
  
  console.log(single[0].title.rendered);
  ...
  return (
    <>
      <SeoMeta
        title={single[0].title.rendered}
        // meta_title={meta_title}
        // description={description}
        // image={single[0].featured_media}
      />
        ...
        <div className="container">
          <div className="row justify-center">
            <article className="lg:col-10">
              ...
              <h1
                dangerouslySetInnerHTML={markdownify(single[0].title.rendered)}
                className="h2 mb-4"
              />
              <ul className="mb-4">
              ...
              <div className="content mb-10">
                <MDXContent content={single[0].content.rendered} />
              </div>
